module NavigationTags
  ...
  tag "sub-nav" do |tag|
    ...
    css_class = ["depth-#{depth}", ("current" if current_page == child_page), ("has_children" if child_page.children.size > 0), ("parent_of_current" if current_page.url.starts_with?(child_page.url) and current_page != child_page)].compact
    if !@first_set
      css_class << 'first'
      @first_set = true
    end
